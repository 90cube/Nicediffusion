...
from ..domains.generation.modes import Txt2ImgMode, Img2ImgMode, UpscaleMode
from .logger import (
...
class StateManager:
    ...
    def __init__(self):
        ...
        # 생성 모드 인스턴스들
        self.txt2img_mode = Txt2ImgMode(None, self.device)
        self.img2img_mode = Img2ImgMode(None, self.device)
        self.upscale_mode = UpscaleMode(self.device)
        
        ui_emoji(f"사용 중인 디바이스: {self.device}")
    ...
    async def initialize(self):
        ...
        if await asyncio.to_thread(config_path.exists):
            ...
            warning_emoji(r"config.toml이 없어 기본 경로를 사용합니다.")
    ...
    async def load_model_pipeline(self, model_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            info_emoji(r"파라미터 자동 적용 비활성화 (Top_bar에서는 체크포인트와 VAE만 선택)")
            ...
            if pipeline and hasattr(pipeline, 'tokenizer'):
                ...
                success(f"LongPromptHandler 초기화: {model_type} 모드 (최대 {max_tokens} 토큰)")
            
            success(f"PromptProcessor 업데이트: {model_type} 모드 (최대 {self.prompt_processor.max_tokens} 토큰)")
    ...
    async def _load_model_heavy_work(self, model_info: Dict[str, Any]):
        ...
        def _load():
            ...
            if model_type == 'SD15':
                ...
                success(r"SD15 품질 최적화 설정 적용 완료")
            else:
                # SDXL 설정
                if hasattr(pipeline, 'enable_attention_slicing'):
        ...
        success(f"모델 로딩 완료: {model_info['name']}")

    async def _unload_current_model(self):
        """기존 모델을 메모리에서 해제"""
        if self.pipeline is not None:
            info(r"🗑️ 기존 모델 언로드 중...")
            try:
            ...
            for folder_vae in available_vae.values():
                for vae_info in folder_vae:
                    if recommended_vae.lower() in vae_info['name'].lower():
                        success(f"추천 VAE 발견: {vae_info['name']}")
                ...
                for folder_vae in available_vae.values():
                    for vae_info in folder_vae:
                        if preferred_name.lower() in vae_info['name'].lower():
                            success(f"SD15 권장 VAE 발견: {vae_info['name']}")
                            await self.load_vae(vae_info['path'])
                            return
        
        # 기본적으로 내장 VAE 사용
        info_emoji(r"체크포인트 내장 VAE 사용 (별도 VAE 없음)")
    ...
    async def generate_image(self):
        ...
        try:
            ...
            if current_mode in ['img2img', 'inpaint', 'upscale']:
                ...
                if not init_image:
                    failure(r"img2img 모드에서 원본 이미지가 없습니다")
                    self._notify_user('이미지를 먼저 업로드해주세요.', 'warning')
                    return
                
                success(f"원본 이미지 보존 확인: {init_image.size}")
                # 원본 이미지를 파라미터에 추가
            ...
            if result and result.images:
                success(f"생성 완료: {len(result.images)}개 이미지")
    ...
    async def _execute_generation(self, pipeline, params: GenerationParams, current_mode: str):
        ...
        for forbidden in ['current_model_info', 'current_loras', 'current_vae_path', 'preview', 'preview_image']:
            if forbidden in params_dict:
                warning_emoji(f"경고: 생성 파라미터에 외부 상태({forbidden})가 포함되어 있음. 무시합니다.")
        ...
        if current_mode == 'upscale':
            result = await self._execute_upscale_generation(params, params_dict)
        else:
            result = await strategy.execute(params_dict, model_info)
        
        # 1단계: 생성 완료 후 이미지가 StateManager에 저장되는지 확인
        info("=" * 80)
        debug_emoji(r"[디버깅 1단계] 생성 완료 후 이미지 저장 확인")
        info("=" * 80)
        
        if result.success and result.images:
            success(f"생성 성공: {len(result.images)}개 이미지")
            info(f"   - result.images 타입: {type(result.images)}")
            info(f"   - result.images[0] 타입: {type(result.images[0])}")
            info(f"   - result.images[0] 크기: {result.images[0].size if hasattr(result.images[0], 'size') else 'N/A'}")
            
            # 생성된 이미지를 독립적으로 저장 (UI 동기화 강화)
            generated_images = result.images
            self.set_generated_images(generated_images)
            success(r"StateManager에 generated_images 독립 저장 완료")
            info(f"   - 저장된 이미지 개수: {len(generated_images)}")
            
            # 원본 이미지 보존 확인 및 강화
            self.preserve_init_image()
            self.ensure_image_state_preservation()
            
            # 각 이미지별 후처리 (저장 및 히스토리)
            for i, image in enumerate(generated_images):
                info(f"   - 이미지 {i+1} 후처리 시작")
                await self.finish_generation(image, params, params_dict['seed'])
                info(f"   - 이미지 {i+1} 후처리 완료")
            
            # 최종 이벤트 발생 (Canvas 프리뷰용)
    ...
    async def _execute_upscale_generation(self, params: GenerationParams, params_dict: dict):
        """Upscale 모드 전용 생성 로직"""
        try:
            ...
            init_image = self.get('init_image')
            if not init_image:
                failure("업스케일 모드에서 원본 이미지가 없습니다")
                return None
            
            # 업스케일 파라미터 가져오기
            upscale_method = getattr(params, 'upscale_method', 'AI Upscale')
            upscale_factor = getattr(params, 'upscale_factor', 2.0)
            
            if upscale_method == 'AI Upscale':
                # AI 업스케일링
                upscale_strength = getattr(params, 'upscale_strength', 0.8)
                upscale_steps = getattr(params, 'upscale_steps', 20)
                
                result = await self.upscale_mode.ai_upscale(
                    image=init_image,
                    prompt=params.prompt,
                    negative_prompt=params.negative_prompt,
                ...
                # 간단한 업스케일링
                simple_method = getattr(params, 'simple_method', 'Bicubic')
                result = await self.upscale_mode.simple_upscale(
                    image=init_image,
                    scale_factor=upscale_factor,
                    method=simple_method
                )
                result = [result]  # 단일 이미지를 리스트로 변환
            
            if result:
                success(f"업스케일 생성 완료: {len(result)}개 이미지")
                return type('Result', (), {
                    'success': True,
                    'images': result,
                    'errors': []
                ...
                return type('Result', (), {
                    'success': False,
                    'images': [],
                    'errors': ['업스케일 생성에 실패했습니다']
                })()
                
        except Exception as e:
            failure(f"업스케일 생성 중 오류: {e}")
            return type('Result', (), {
                'success': False,
                'images': [],
                'errors': [str(e)]
    ...
    def apply_params_from_metadata(self, model_info: Dict[str, Any], include_prompts: bool = False):
        ...
        if include_prompts:
            ...
            info_emoji(r"프롬프트는 제외하고 파라미터만 적용합니다.")
        ...
        if seed is not None and seed != -1:
            ...
            success(f"Seed {seed} 발견, 랜덤 시드로 설정: -1")
        else:
            self.update_param('seed', -1)
        
        success(r"메타데이터에서 파라미터가 적용되었습니다.")
            ...
            if current_image is not None and image is not None:
                if hasattr(current_image, 'size') and hasattr(image, 'size'):
                    if current_image.size == image.size:
                        warning_emoji(r"set_init_image 중복 호출 방지됨 (동일한 크기)")
                        return
            # 다른 이미지인 경우에는 허용
            info_emoji(r"다른 이미지로 업데이트 허용")
        
        try:
            self._setting_init_image = True
            
            if image is not None:
                debug_emoji(f"이미지 정보: 크기={image.size}, 모드={image.mode}")
                # 원본 이미지를 영구 보존
                self._state['init_image'] = image
                success(f"init_image 영구 보존 완료: {image.size}")
                
                # 저장 후 확인
                saved_image = self._state.get('init_image')
                debug_emoji(f"저장 후 확인: 타입={type(saved_image)}, 크기={saved_image.size if saved_image else 'None'}")
                
                # 이벤트 발생 (이미지 프리뷰 강화)
                self._notify('init_image_changed', {'status': 'success', 'size': image.size})
                success(r"init_image_changed 이벤트 발생")
            else:
                warning_emoji(r"init_image가 None으로 설정됨")
                self._state['init_image'] = None
                
                # 이벤트 발생
                self._notify('init_image_changed', {'status': 'cleared'})
                success(r"init_image_changed 이벤트 발생 (cleared)")
                    
        except Exception as e:
            failure(f"set_init_image 오류: {e}")
            import traceback
            traceback.print_exc()
        finally:
            # 플래그 해제 (이미지 프리뷰 강화)
    ...
    def set_generated_images(self, images):
        ...
        """
        debug_emoji(f"set_generated_images 호출: 이미지 개수={len(images) if images else 0}")
        
        # 무한 재귀 방지 플래그 확인
        if hasattr(self, '_setting_generated_images') and self._setting_generated_images:
            warning_emoji(r"set_generated_images 중복 호출 방지됨")
            return
        ...
        try:
            ...
            if images:
                ...
                for i, img in enumerate(images):
                    debug_emoji(f"생성된 이미지 {i+1}: 크기={img.size}, 타입={type(img)}")
                    # 썸네일 크기인지 확인 (150x150)
                    if img.size == (150, 150):
                        warning_emoji(r"경고: 썸네일 크기(150x150)가 StateManager에 저장됨 - 원본 이미지가 필요함")
                ...
                success(r"generated_images_changed 이벤트 발생")
            else:
                self._state['generated_images'] = []
                
                # 이벤트 발생
                self._notify('generated_images_changed', {'count': 0})
                success(r"generated_images_changed 이벤트 발생 (cleared)")
                    
        except Exception as e:
            failure(f"set_generated_images 오류: {e}")
        finally:
    ...
    def clear_generated_images(self):
        ...
        if not hasattr(self, '_clearing_generated_images') or not self._clearing_generated_images:
            try:
                ...
                # 이벤트 발생 (UI 동기화 강화)
                self._notify('generated_images_changed', {'count': 0})
                success(r"generated_images 초기화 완료")
            except Exception as e:
                failure(f"clear_generated_images 오류: {e}")
            finally:
    ...
    def preserve_init_image(self):
        ...
        if init_image:
            ...
            warning_emoji(r"원본 이미지가 없음 - 보존할 이미지 없음")
            return False
    
    def ensure_image_state_preservation(self):
        """이미지 상태 보존 강화 - 생성 전후 호출"""
        try:
            process_emoji(r"이미지 상태 보존 강화 시작")
            
            # 원본 이미지 보존 확인
            init_image = self.get('init_image')
            if init_image:
                success(f"원본 이미지 보존됨: {init_image.size}")
            else:
            ...
            if generated_images:
                success(f"생성된 이미지 보존됨: {len(generated_images)}개")
            else:
                info_emoji(r"생성된 이미지 없음")
            
            success(r"이미지 상태 보존 강화 완료")
            
        except Exception as e:
            failure(f"이미지 상태 보존 강화 중 오류: {e}")
        ...
        if event in self._observers:
            try:
                if callback in self._observers[event]:
                    ...
                    success(f"이벤트 '{event}' 구독 해제 완료")
                else:
                    warning_emoji(f"이벤트 '{event}'에서 콜백을 찾을 수 없음 (이미 해제됨)")
            except ValueError:
                warning_emoji(f"이벤트 '{event}' 구독 해제 중 오류 (콜백이 리스트에 없음)")
            except Exception as e:
                failure(f"이벤트 '{event}' 구독 해제 중 예상치 못한 오류: {e}")
    ...
    def _notify(self, event: str, data: Any = None):
        ...
        if event in self._observers:
            ...
            if hasattr(self, '_last_event_time') and event in self._last_event_time:
                if current_time - self._last_event_time[event] < 0.1:  # 100ms 내 중복 방지
                    return
            ...
            for callback in self._observers[event]:
                try:
                    ...
                    warning_emoji(f"이벤트 콜백 오류 ({event}): {e}")
    ...
    def copy_prompt_to_clipboard(self, positive_prompt: str = "", negative_prompt: str = ""):
        ...
        try:
            ...
            info(f"📋 클립보드에 복사됨: {full_prompt[:50]}...")
            self._notify_user("프롬프트가 클립보드에 복사되었습니다.", 'positive')
        except Exception as e:
            failure(f"클립보드 복사 실패: {e}")
    ...
    def _add_to_history(self, history_item: Dict[str, Any]):
        ...
        info(f"📋 히스토리에 추가됨: {history_item.get('model', 'Unknown')}")
